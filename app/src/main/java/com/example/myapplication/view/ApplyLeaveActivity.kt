...
class ApplyLeaveActivity : AppCompatActivity() {
    ...
    private fun updateDayTypeVisibility(days: Int) {
        val options = if (days == 1) listOf("Full Day", "Half Day") else listOf("Full Day")
        ...
        autoCompleteDayType.setOnItemClickListener { _, _, _, _ ->
            rgHalfDay.visibility = if (autoCompleteDayType.text.toString() == "Half Day") View.VISIBLE else View.GONE
        }
    }
    ...
    private fun validateAndApply() {
        ...
        val dayParam = if (autoCompleteDayType.text.toString() == "Half Day")
